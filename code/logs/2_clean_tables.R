...
wave_df <- read_csv(wave, skip_empty_rows = T, col_names = T)
...
# if (wave %in% c('30', '29', '31', '32', '34') & str_detect(wave_df[1, 2], 'LIST OF')) {
#   l <- slice(l, 2:nrow(l)) }
...
write_csv(l,
  '/Dropbox/State_Reach/DATA/ANQAR/Replaced_locations/Extracted_tables_parts' %p%
    wave %p% '.csv')
